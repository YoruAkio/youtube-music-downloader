...
# Check if the youtube_downloader package is available
try:
    from youtube_downloader.__main__ import main as package_main
    from youtube_downloader import __version__
    
    # Run the application using the package
    if __name__ == "__main__":
        sys.exit(package_main())
except ImportError:
    # If the package is not available, we'll run the legacy version
    console.print("[bold yellow]Running in legacy mode as the youtube_downloader package is not available.[/bold yellow]")
    
    # Legacy code starts here - copied from original implementation
    ...
    # Track active tasks for better progress visualization
    class ProgressTracker:
        def __init__(self, progress, max_visible=3):
    ...
    def main():
        ...
        # Validate the URL
        url_type = validate_url(args.url)
        if not url_type:
        ...
        # Process the downloads with a progress bar
        with Progress(
            SpinnerColumn(),
            ...
            TimeElapsedColumn(),
            TimeRemainingColumn(),  # Added remaining time estimate
            console=console,
            expand=True,  # Allow progress bar to expand to full width
            refresh_per_second=10  # More frequent updates
        ) as progress:
            # Create a progress tracker to manage visible tasks
            progress_tracker = ProgressTracker(progress, max_visible=5)
            
            start_time = time.time()
            console.print("\n[bold cyan]Starting downloads...[/bold cyan]")
            
            results = process_url(args.url, args, progress, progress_tracker)
            ...
            if not args.video and any(r['status'] == 'success' for r in results):
                ...
                conversion_results = []
                max_workers = args.parallel_convert
                
                # Create new progress tracker for conversions
                progress_tracker = ProgressTracker(progress, max_visible=5)
                
                if max_workers == 0:  # Sequential processing
                    for idx, file_info in enumerate([r for r in results if r['status'] == 'success'], 1):
                        basename = os.path.basename(file_info['file'])
                        task_id = progress_tracker.add_task(f"[yellow]Converting: {basename}[/yellow]", total=100)
                        
                        conversion_results.append(convert_file(file_info, args, progress, task_id, progress_tracker))
                else:  # Parallel processing
                    with ThreadPoolExecutor(max_workers=max_workers) as executor:
                        futures = []
                        
                        # Submit all conversion jobs
                        for file_info in [r for r in results if r['status'] == 'success']:
                            basename = os.path.basename(file_info['file'])
                            task_id = progress_tracker.add_task(f"[yellow]Converting: {basename}[/yellow]", total=100)
        ...
        if success_count > 0:
            ...
            for result in [r for r in results if r['status'] == 'success']:
                console.print(f"[cyan]- {result['title']}[/cyan] -> [green]{result['file']}[/green]")
        
        if skipped_count > 0:
            console.print("\n[bold yellow]Skipped files (already exist):[/bold yellow]")
            for result in [r for r in results if r['status'] == 'skipped']:
                console.print(f"[cyan]- {result['title']}[/cyan] -> [yellow]{result['file']}[/yellow]")
        
        if error_count > 0:
            console.print("\n[bold red]Failed downloads:[/bold red]")
            for result in [r for r in results if r['status'] == 'error']:
                console.print(f"[red]- {result.get('title', result['url'])}: {result['error']}[/red]")
